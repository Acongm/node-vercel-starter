-- Add configurable path ranking dimension for APM slow-interface discovery.

drop function if exists public.admin_api_request_log_stats(timestamptz, int, boolean);
...
revoke all on function public.admin_api_request_log_stats(timestamptz, int, boolean, text) from public, anon, authenticated;
grant execute on function public.admin_api_request_log_stats(timestamptz, int, boolean, text) to service_role;
